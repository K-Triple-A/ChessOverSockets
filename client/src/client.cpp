...
{
    int temp = 1;
    if (send(gstFD, &temp, sizeof(int), 0) <= 0)   // send to server and wait server reply with another player fd and name
    {
        return -1;
    }
...
{
    // 🔧 FIXED: Remove hardcoded row, use current king row
    int row = kingspt.x;
    if ((to.x == row) && (to.y == 0 || to.y == 7))
    {
        if (kingspt.y < to.y)
        {
            ...
            for (int j = kingspt.y + 1; j < to.y; j++)
            {
                if (!safe_spot({row, j})) return false;
            }
            swap(board[row][kingspt.y], board[row][6]);
            swap(board[to.x][to.y], board[row][5]);
            kingspt = {row, 6};
            if (!safe_spot(kingspt))
            {
                swap(board[row][6], board[row][kingspt.y]);
                swap(board[row][5], board[to.x][to.y]);
                kingspt = {row, 4};
                return false;
            }
        }
        else
        {
            ...
            swap(board[row][kingspt.y], board[row][2]);
            swap(board[to.x][to.y], board[row][3]);
            kingspt = {row, 2};
            if (!safe_spot(kingspt))
            {
                swap(board[row][2], board[row][kingspt.y]);
                swap(board[row][3], board[to.x][to.y]);
                kingspt = {row, 4};
                return false;
            }
        }
...
bool Chess::safe_spot(spot spt)
{
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < n; j++)
        {
            if (board[i][j] && board[i][j]->get_color() == enemy)
            {
                ...
                {
                    // 🔧 FIXED: Handle pawn threats based on enemy color
                    if (enemy == white && i - spt.x == 1 && abs(spt.y - j) == 1)
                        return false;
                    if (enemy == black && spt.x - i == 1 && abs(spt.y - j) == 1)
                        return false;
                }
            }
        }
...
//********************************
void Chess::sendmv(pieceMove mv)
{
    send(gstFD, &mv, sizeof(mv), 0);
}
int Chess::recvmv()
{
    ...
    {
        bool accept_draw;
        cout<<guest_name<<" is offering draw"<<endl;
        cout<<"if you accept it enter 1 otherwise 0 : ";
        cin>>accept_draw;
        if(accept_draw)
        {
        mode = draw;
            ...
            board[to.x][to.y]->get_type() == rook)
    {
        ...
        kingspt = {0, (from.y < to.y ? 6 : 2)}; // ✅ ADDED: Update opponent king position
        update_status(); // ✅ ADDED
        return 1;
    }

    if (board[to.x][to.y] &&
            board[to.x][to.y]->get_color() == player)
    {
        chess_piece::killed[player][board[to.x][to.y]->get_type()]++;
        delete board[to.x][to.y];
    ...
    // 🔧 FIXED: Track king movement from opponent side
    if (board[to.x][to.y]->get_type() == king)
    {
        kingspt = to;
    }
...
void Chess::CleanUP()
{
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < n; j++)
        {
            ...
            {
                delete board[i][j];
                board[i][j] = nullptr;  // <== 🔧 This is crucial
            }
        }
    }
}
